...
void GameManager::sendNextQuestion_UNLOCKED() {
  // Round Limit Check
  int limit = 5; // Round 1
  if (m_current_round == 2) limit = 3;
  else if (m_current_round == 3) limit = 2;
  ...
  auto copy_opt = [&](int i, const std::string &key) {
    if (m_current_question.options.count(key))
      std::strncpy(q.options[i], m_current_question.options.at(key).c_str(), 63);
  };
  
  // Set question type based on round
  if (m_current_round == 1) {
      q.question_type = protocol::QT_MCQ;
      copy_opt(0, "A"); copy_opt(1, "B"); copy_opt(2, "C"); copy_opt(3, "D");
...
void GameManager::handleSubmitAnswer_UNLOCKED(
  ...
  if (m_current_round == 1) {
      if (!ans.empty()) ans = ans.substr(0, 1);
  }
...
void GameManager::processRoundResults_UNLOCKED() {
  std::cout << "[GameManager] Processing Results (Round " << m_current_round << ")..." << std::endl;
  
  // Calculate Time Factor (Speed Score)
  auto now = std::chrono::steady_clock::now();
  int duration_ms = std::chrono::duration_cast<std::chrono::milliseconds>(now - m_question_start_time).count();
  ...
  for (int s : m_active_players) {
    ...
    if (m_current_round == 1) {
        ...
        double accuracy = calculateAccuracy(user_ans, m_current_question.correct_answer);
        points = 100 * accuracy * time_factor;
        if (accuracy > 0.8) is_correct = true; // Visual feedback
    } else {
        // Round 3: Estimation
        long long target = 0;
        try { target = std::stoll(m_current_question.correct_answer); } catch(...) {}
        
        long long user_val = 0;
        try { user_val = std::stoll(user_ans); } catch(...) {}
...
void GameManager::eliminatePlayers_UNLOCKED() {
    ...
    if (m_current_round == 1) keep_count = 3; // Keep Top 3
    else if (m_current_round == 2) keep_count = 2; // Keep Top 2
    ...
    for (int s : eliminated) {
        m_active_players.erase(s);
        protocol::MessagePacket msg;
        std::strcpy(msg.message, "[ELIMINATED] You did not qualify for the next round.");
        ...
        m_room->broadcast_UNLOCKED(protocol::CMD_INFO, &announce, sizeof(announce), s);
    }
}
...
void GameManager::transitionRound_UNLOCKED() {
    ...
    for(auto& pair : m_scores) pair.second = 0;
    
    std::string round_name = (m_current_round == 2) ? "ROUND 2: SEMANTIC TEXT (Top 2 qualify)" : "ROUND 3: ESTIMATION FINAL";
    protocol::MessagePacket msg;
...
double GameManager::calculateAccuracy(const std::string& s1, const std::string& s2) {
