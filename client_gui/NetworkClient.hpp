...
public:
  ...
  template <typename T> void sendData(uint16_t type, const T &payload) {
    if (m_running)
      protocol::sendPacket(m_sock, type, &payload, sizeof(T));
  }

  void sendCommand(uint16_t type) {
    if (m_running)
      protocol::sendPacket(m_sock, type, nullptr, 0);
  }
  ...
  void sendCreateRoom(const std::string &name, bool is_ranked = false) {
    protocol::RoomReqPacket pkt;
    memset(&pkt, 0, sizeof(pkt));
    strncpy(pkt.room_name, name.c_str(), 63);
    pkt.is_ranked = is_ranked ? 1 : 0;
  ...
  void sendGetHistory() { sendCommand(protocol::CMD_GET_HISTORY); }

  void sendAddBot(int count) {
    if (m_running)
      protocol::sendPacket(m_sock, protocol::CMD_ADD_BOT, &count,
                           sizeof(count));
  }
